Development of jkp-naissus plugin
===========================

This project uses [qgis_plugin_tools](https://github.com/NMItools/qgis_plugin_tools) submodule,
so set git setting value: `git config --global submodule.recurse true`.

When cloning use `--recurse-submodules` like so:
`git clone --recurse-submodules https://github.com/NMItools/jkp-naissus.git`
...
The code for the plugin is in the [jkpnaissus](../jkp-naissus) folder. Make sure you have required tools, such as
Qt with Qt Editor and Qt Linquist installed by following this
[tutorial](https://www.qgistutorials.com/en/docs/3/building_a_python_plugin.html#get-the-tools).

For building the plugin use platform independent [build.py](../jkp-naissus/build.py) script.

## Setting up development environment

To get started with the development, follow these steps:

1. Go to the  [jkpnaissus](../jkp-naissus) directory with a terminal
...
* they contain absolute imports:
    ```python
    from jkpnaissus.utils.exceptions import TestException # Good

    from ..utils.exceptions import TestException # Bad

    ```
* they will be found by [build.py](../jkpnaissus/build.py) script (`py_files` and `ui_files` values)

* you consider adding test files for the new functionality
## Deployment

Edit [build.py](../jkpnaissus/build.py) to contain working values for *profile*, *lrelease* and *pyrcc*. If you are
...
* Add this badge ![](https://github.com/NMItools/jkp-naissus/workflows/Translations/badge.svg) to
...
The translation files are in [i18n](../jkp-naissus/resources/i18n) folder. Translatable content in python files is
